    ...
    CreateEvent: (cb) => {
      let watcher = eventContractInstance.CreateEvent((error, result) => {
        if (error) {
          ...
          console.log('  Current number of attendees: ' + result.args._numAttendees.toString());
          console.log(`  Event createDateTime: ${new Date(parseInt(result.args._eventCreateDateTime.toString(), 10))}`);
          console.log(`  Event startDateTime: ${new Date(parseInt(result.args._eventStartDateTime.toString(), 10))}`);
          console.log(`  Event endDateTime: ${new Date(parseInt(result.args._eventEndDateTime.toString(), 10))}`);
        }
